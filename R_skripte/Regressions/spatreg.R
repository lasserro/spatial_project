...
for (i in 1:length(period)) {
  sar[[i]] <- lagsarlm(f1, data=data.long[data.long$time==period[i],], W.list.k, tol.solve=1.0e-30)
}
names(sar) <- period
...
sem <- list()
for (i in 1:length(period)) {
  sem[[i]] <- errorsarlm(f1, data=data.long[data.long$time==period[i],], W.list.k, tol.solve=1.0e-30)
}
names(sem) <- period
...
sdm <- list()
for (i in 1:length(period)) {
  sdm[[i]] <- lagsarlm(f1, data=data.long[data.long$time==period[i],], W.list.k, type="mixed",  tol.solve=1.0e-30)
}
names(sdm) <- period

#direct indirect and total effects
for (i in 1:k) {
sar.impacts[[i]]<-impacts(sar[[i]], listw = W.list.k)
sdm.impacts[[i]]<-impacts(sdm[[i]], listw = W.list.k)
#sem.impacts[[i]]<-impacts(sem[[i]], listw = W.list.k)
}
names(sar.impacts) <- period
names(sdm.impacts) <- period
#names(sem.impacts) <- period